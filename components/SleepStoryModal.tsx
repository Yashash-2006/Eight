...
import { SLEEP_STORY_THEMES } from '../constants';
import LoadingSpinner from './LoadingSpinner';

interface SleepStoryModalProps {
  onSelectTheme: (theme: string) => void;
  onCancel: () => void;
  isLoading: boolean;
}

const SleepStoryModal: React.FC<SleepStoryModalProps> = ({ onSelectTheme, onCancel, isLoading }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 transition-opacity">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 w-full max-w-lg text-center transform transition-all scale-100">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Sleep Stories</h2>
        <p className="text-gray-600 dark:text-gray-400 mb-8">Choose a theme for a calming story to help you unwind.</p>

        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-48">
            <LoadingSpinner />
            <p className="mt-4 text-gray-500 dark:text-gray-400">Crafting your story...</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-4 mb-8">
              {SLEEP_STORY_THEMES.map((theme) => (
                <button
                  key={theme.id}
                ...
                >
                  <Icon name={theme.icon as any} className="w-10 h-10 text-indigo-600 dark:text-indigo-400 transition-transform group-hover:scale-110" />
...
export default SleepStoryModal;
